...
function setQuick(m,el){viewMode=m;customDate=null;customDateEnd=null;ordersPage=1;leadsPage=1;document.querySelectorAll('#period-row .seg-btn').forEach(function(b){b.classList.remove('active');});el.classList.add('active');var di=document.getElementById('custom-date');if(di)di.value='';var de=document.getElementById('custom-date-end');if(de)de.value='';resetRangeUI();loadAll();}
// El rango de fechas vive plegado detrás del segmento "Fechas": sin cajas vacías a la vista
function toggleRange(){var r=document.getElementById('range-row');if(!r)return;var open=r.classList.toggle('open');if(open){var di=document.getElementById('custom-date');if(di)di.focus();}}
function resetRangeUI(){var r=document.getElementById('range-row');if(r)r.classList.remove('open');var t=document.getElementById('seg-fechas-t');if(t)t.textContent='Fechas';}
function fmtShortDate(v){var p=v.split('-');return parseInt(p[2],10)+' '+MESES[parseInt(p[1],10)-1].toLowerCase();}
...
var sf=document.getElementById('seg-fechas');if(sf)sf.classList.add('active');
var st=document.getElementById('seg-fechas-t');if(st)st.textContent=fmtShortDate(s)+(e?' → '+fmtShortDate(e):'');
loadAll();}
...
function filteredOrders(){var f=filterByDate(allOrders);var sf=document.getElementById('filter-status').value;var tf=document.getElementById('filter-type').value;if(sf!=='all')f=f.filter(function(o){return o.status===sf;});if(tf!=='all')f=f.filter(function(o){return o.order_type===tf;});
